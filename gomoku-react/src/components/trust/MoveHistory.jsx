...
export default function MoveHistory({ moveHistory, genesisHash }) {
      ...
      >
        {moveHistory.length === 0 ? (
          ...
          moveHistory.map((m, idx) => (
            <div
              key={m.num}
              className={`flex items-center gap-2 px-2 py-1.5 font-mono text-[12px] rounded-sm transition-colors hover:bg-ink/[0.04] ${idx === moveHistory.length - 1 ? 'bg-ink/[0.05] text-ink' : 'text-ink-faint'}`}
            >
